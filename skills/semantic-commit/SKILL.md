...
- **Never mention Claude, an AI assistant, or a session/tool in the commit message** — no `Co-Authored-By`, no session links, no "generated by" notes. The message is exactly `<type>: <subject>`, nothing else
